...
import test.ru.oooinex.autotests.buyticket.testitself.ReturnTrainPagesTest;
...
public class ReturnTrainTest {
	static ReturnTrainPagesTest rzdPage;
	...
	public static void setUp(String baseUrl, String holdBrowser, String browser, String profile, String reportsFolder, String reportsUrl, String timeout, String paymentPlug) throws IOException {
		if (driver == null) {
			...
			rzdPage = open(url, ReturnTrainPagesTest.class);
	...
	@Test(description = "Заполняем стартовую форму и нажимаем 'Купить билет' или 'Расписание' в зависимости от флага 'Только с билетами'", 
		  dataProvider = "DirectTrainWithReturn", dataProviderClass = TrainDataProvider.class, dependsOnMethods = { "Login" })
	...
	@Test(description = "Выбираем нужный поезд по номеру и нажимаем 'Продолжить'. Должен появиться список вагонов", 
		  dataProvider = "DirectTrainWithReturn", dataProviderClass = TrainDataProvider.class, 
		  dependsOnMethods = { "FillStartForm" })
	public void SelectDirectTrainWithReturn(String trainNumber, String carCheckers, String trainNumberReturn, String carCheckersReturn) {
		// Если список поездов появился, выбираем нужный и нажимаем 'Продолжить'
		rzdPage.trainWithReturnSelect(trainNumber, carCheckers, trainNumberReturn, carCheckersReturn).result().shouldBe(CollectionCondition.sizeGreaterThan(0));
	}

	@Test(description = "Выбираем нужный вагон по номеру и нажимаем 'Продолжить'. Должен появиться список пассажиров", 
		  dataProvider = "DirectTrainWithReturn", dataProviderClass = TrainDataProvider.class, 
		  dependsOnMethods = { "SelectDirectTrainWithReturn" })
	public void SelectWagonDirectTrainWithReturn(String wagonType, String wagonCategory, String servicesAndCapabilities, String returnWagonType, String returnWagonCategory, String returnServicesAndCapabilities) {
		// Если список вагонов появился выбираем нужный и продолжить
		rzdPage.wagonSelect(wagonType, wagonCategory, servicesAndCapabilities, returnWagonType, returnWagonCategory, returnServicesAndCapabilities).result().shouldHave(text("Список пассажиров"));
	}

	@Test(description = "Заполняем все формы пассажиров необходимыми параметрами", 
		  dataProvider = "DirectTrainWithReturn", dataProviderClass = TrainDataProvider.class, 
		  dependsOnMethods = { "SelectWagonDirectTrainWithReturn" })
	...
	public void FillPlaceSigns(Object[] directPlaces, Object[] returnPlaces) {
		// Если список пассажиров появился заполняем все формы
		rzdPage.fillPlaceSigns(directPlaces, returnPlaces).result().shouldHave(text("Проверка данных пассажиров"));
	}
	...
	@Test(description = "Отмена электронной регистрации и возврат билета", 
		  dataProvider = "DirectTrainWithReturn", dataProviderClass = TrainDataProvider.class, dependsOnMethods = { "CompleteOrderProcessing" })
